...
{
	global $sourcedir, $modSettings, $user_info, $pmxcFunc;
...
function registerMember(&$regOptions, $return_errors = false)
{
	...
	if ($pmxcFunc['db_num_rows']($request) != 0)
		$reg_errors[] = array('lang', 'email_in_use', false, array($pmxcFunc['htmlspecialchars']($regOptions['email'])));
	...
	$memberID = $pmxcFunc['db_insert_id']('{db_prefix}members', 'id_member');
	...
	if (!allowedTo('moderate_forum') && ((!empty($modSettings['reserveName']) && $is_name) || !empty($modSettings['reserveUser']) && !$is_name))
	{
		...
		foreach ($reservedNames as $reserved)
		{
			...
			if (empty($modSettings['reserveCase']))
				$reservedCheck = $pmxcFunc['strtolower']($reservedCheck);
...
function groupsAllowedTo($permission, $board_id = null)
{
	...
	else
	{
		...
		elseif ($board_id !== 0)
		{
			...
			list ($profile_id) = $pmxcFunc['db_fetch_row']($request);
			$pmxcFunc['db_free_result']($request);
		}
		else
			$profile_id = 1;
...
function reattributePosts($memID, $email = false, $membername = false, $post_count = false)
{
	...
	if ($email === false && $membername === false)
	{
		...
		list ($email, $membername) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
	}
	...
	if ($post_count)
	{
		...
		list ($messageCount) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
	...
	$updated['messages'] = $pmxcFunc['db_affected_rows']();
	...
	if ($updated['messages'] > 0)
	{
		...
		$updated['topics'] = $pmxcFunc['db_affected_rows']();
		...
		$updated['reports'] = $pmxcFunc['db_affected_rows']();
	}
...
function generateValidationCode()
{
	...
	$request = $pmxcFunc['db_query']('get_random_number', '
		SELECT RAND()',
		array(
		)
	);

	list ($dbRand) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
