...
{
	global $txt, $scripturl, $context, $settings, $modSettings, $pmxcFunc;
	...
	list ($num_members) = $pmxcFunc['db_fetch_row']($request);
...
{
	global $context, $txt, $pmxcFunc, $sourcedir, $cat_tree, $boardList, $boards;
...
{
	global $context, $txt, $pmxcFunc, $modSettings;
	...
	if ($context['group']['id'] > 0)
	{
		...
		list ($context['group']['name'], $parent) = $pmxcFunc['db_fetch_row']($result);
...
function ModifyMembergroup2()
{
	...
	else
	{
		...
		list ($parent) = $pmxcFunc['db_fetch_row']($result);
		$pmxcFunc['db_free_result']($result);
	}
...
{
	global $context, $modSettings, $sourcedir, $txt, $scripturl, $pmxcFunc;
...
function setPermissionLevel($level, $group, $profile = 'null')
{
	...
	elseif ($profile !== 'null' && $group === 'null' && ($profile == 1 || $profile > 4))
	{
		...
		while ($row = $pmxcFunc['db_fetch_row']($query))
		{
			$group = $row[0];
...
function loadAllPermissions()
{
	...
	// Note to Mod authors - you don't need to stick your permission group here if you don't mind PMX sticking it the last group of the page.
...
function EditPermissionProfiles()
{
	...
	if (isset($_POST['create']) && trim($_POST['profile_name']) != '')
	{
		...
		$_POST['profile_name'] = $pmxcFunc['htmlspecialchars']($_POST['profile_name']);
		...
		$profile_id = $pmxcFunc['db_insert_id']('{db_prefix}permission_profiles', 'id_profile');
	...
	elseif (isset($_POST['rename']))
	{
		...
		else
		{
			...
			{
				$value = $pmxcFunc['htmlspecialchars']($value);
...
{
	global $context, $txt, $pmxcFunc, $modSettings, $sourcedir;
