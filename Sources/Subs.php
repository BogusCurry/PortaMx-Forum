...
function setECL_Cookie()
{
	...
	{
		$parts = eclCookieparts();
		setcookie('eclauth', 'LiPF_cookies_authorised', strtotime('+30 day'), $parts['path'], $parts['host'], !empty($modSettings['secureCookies']), !empty($modSettings['httponlyCookies']));
	}
}
...
function ecl_Init_modal()
{
	...
	ecl_loadvars(true);
	loadtemplate('EclModal');
}

/**
* load need css and scriptvars
*/
function ecl_loadvars($overlay)
{
	...
	var eclInMaintenace = '. (!empty($maintenance) || !empty($overlay) ? 'true' : 'false') .';
	var eclofsTop = '. intval($modSettings['ecl_topofs']) .';
	var eclOverlay = true;');
	loadCSSFile('ecl.css', array());
}
...
function checkECL_Cookie($hideforBots = false)
{
	...
	if(empty($hideforBots) && !empty($user_info['is_guest']) && (!empty($user_info['possibly_robot'])))
		return true;
	else
...
/**
* Get cookie values for ecl
*/
function eclCookieparts()
{
					...
					if (!empty($currentAttachment['is_image']))
					{
						...
						if (isset($_REQUEST['preview']) || !isset($context['lbimage_data']['lightbox_id']) || !empty($noLightbox))
						{
							if (!empty($modSettings['attachmentShowImages']))
								$returnContext = '<img src="'. ($currentAttachment['thumbnail']['has_thumb'] ? $currentAttachment['thumbnail']['href'] : $currentAttachment['href']) .'"'. $alt . $width . $height . $class .' oncontextmenu="return false">';
						}
						else
						{
							if (!empty($modSettings['attachmentShowImages']))
