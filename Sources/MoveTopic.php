...
function MoveTopic()
{
	global $txt, $board, $topic, $user_info, $context, $language, $scripturl, $pmxcFunc, $modSettings, $sourcedir;
	...
	list ($id_member_started, $context['subject'], $context['is_approved']) = $pmxcFunc['db_fetch_row']($request);
...
function MoveTopic2()
{
	...
	list ($id_member_started, $id_first_msg, $context['is_approved']) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	list ($pcounter, $board_name, $subject) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	{
		$_POST['custom_subject'] = strtr($pmxcFunc['htmltrim']($pmxcFunc['htmlspecialchars']($_POST['custom_subject'])), array("\r" => '', "\n" => '', "\t" => ''));
		// Keep checking the length.
		if ($pmxcFunc['strlen']($_POST['custom_subject']) > 100)
			$_POST['custom_subject'] = $pmxcFunc['substr']($_POST['custom_subject'], 0, 100);
	...
	list ($pcounter_from) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
...
function moveTopics($topics, $toBoard)
{
	...
	list ($isSeen) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	if (!empty($modSettings['cache_enable']) && $modSettings['cache_enable'] >= 3)
		foreach ($topics as $topic_id)
			$pmxCacheFunc['drop']('topic_board-' . $topic_id);
...
{
	global $board, $topic, $pmxcFunc, $scripturl;
	...
	else
	{
		...
		list($topic_subject, $board_name) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
