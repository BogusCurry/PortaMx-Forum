...
function SplitIndex()
{
	global $txt, $topic, $context, $pmxcFunc, $modSettings;
	...
	list ($_REQUEST['subname'], $num_replies, $unapproved_posts, $id_first_msg, $approved) = $pmxcFunc['db_fetch_row']($request);
...
{
	global $txt, $topic, $context, $pmxcFunc;
...
{
	global $txt, $scripturl, $topic, $context, $modSettings, $original_msgs, $pmxcFunc, $options;
	...
	if (isset($_REQUEST['xml']))
	{
		...
		if (empty($context['not_selected']['start']) && $pmxcFunc['db_num_rows']($request) <= 1 && $_REQUEST['move'] == 'down')
			$_REQUEST['move'] = '';
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
	...
	for ($counter = 0; $row = $pmxcFunc['db_fetch_assoc']($request); $counter ++)
	{
...
function splitTopic($split1_ID_TOPIC, $splitMessages, $new_subject)
{
	...
	list ($id_board, $split1_approved) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	$split2_ID_TOPIC = $pmxcFunc['db_insert_id']('{db_prefix}topics', 'id_topic');
	if ($split2_ID_TOPIC <= 0)
		fatal_lang_error('cant_insert_topic');

	// Move the messages over to the other topic.
	$new_subject = strtr($pmxcFunc['htmltrim']($pmxcFunc['htmlspecialchars']($new_subject)), array("\r" => '', "\n" => '', "\t" => ''));
	// Check the subject length.
	if ($pmxcFunc['strlen']($new_subject) > 100)
		$new_subject = $pmxcFunc['substr']($new_subject, 0, 100);
	// Valid subject?
	if ($new_subject != '')
	{
		$pmxcFunc['db_query']('', '
...
function MergeIndex()
{
	global $txt, $board, $context, $pmxcFunc, $sourcedir;
	...
	list ($topiccount) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	list ($subject) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
...
function MergeExecute($topics = array())
{
	...
	if ($pmxcFunc['db_num_rows']($request) < 2)
	...
	if ($pmxcFunc['db_num_rows']($request) != count($boards))
		fatal_lang_error('no_board');
	$pmxcFunc['db_free_result']($request);
	...
	{
		$target_subject = strtr($pmxcFunc['htmltrim']($pmxcFunc['htmlspecialchars']($_POST['custom_subject'])), array("\r" => '', "\n" => '', "\t" => ''));
		// Keep checking the length.
		if ($pmxcFunc['strlen']($target_subject) > 100)
			$target_subject = $pmxcFunc['substr']($target_subject, 0, 100);
	...
	list ($member_started) = $pmxcFunc['db_fetch_row']($request);
	list ($member_updated) = $pmxcFunc['db_fetch_row']($request);
	...
	if (isset($_POST['postRedirect']))
	{
		$_POST['reason'] = $pmxcFunc['htmlspecialchars']($_POST['reason'], ENT_QUOTES);
	...
	list($id_board) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
