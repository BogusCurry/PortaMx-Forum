...
function LockTopic()
{
	global $topic, $user_info, $sourcedir, $board, $pmxcFunc;
	...
	list ($starter, $locked) = $pmxcFunc['db_fetch_row']($request);
...
function Sticky()
{
	global $topic, $board, $sourcedir, $pmxcFunc;
	...
	list ($is_sticky) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
