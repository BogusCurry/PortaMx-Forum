	...
	$listOptions = array(
		...
		'columns' => array(
			'filename' => array(
				...
				'data' => array(
					'sprintf' => array(
						'format' => '<a href="' . $scripturl . '?action=dlattach;topic=%1$d.0;attach=%2$d%4$s" %5$s>%3$s</a>',
						'params' => array(
							'topic' => true,
							'id' => true,
							'filename' => false,
...
function list_getAttachments($start, $items_per_page, $sort, $boardsAllowed, $memID)
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		...
		$attachments[] = array(
			...
			'image' => $isImage ? ';image' : '',
			'lbdata' => $isImage ? ' class=lb-link title='. $txt['lightbox_expand_short'] .' data-title=' . $row['filename'] .' data-lightbox=manage-attach' : '',
