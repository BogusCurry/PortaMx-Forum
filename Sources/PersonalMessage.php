...
function MessageMain()
{
	...
	elseif (($context['message_limit'] = cache_get_data('msgLimit:' . $user_info['id'], 360)) === null)
	{
		...
		list ($maxMessage, $minMessage) = $pmxcFunc['db_fetch_row']($request);
...
{
	global $context, $modSettings, $pmxcFunc, $memberContext, $scripturl, $user_settings, $db_show_debug;
...
function MessageFolder()
{
	...
	global $messages_request, $user_info, $recipients, $options, $pmxcFunc, $user_settings;
	...
	list ($max_messages) = $pmxcFunc['db_fetch_row']($request);
	$pmxcFunc['db_free_result']($request);
	...
	if (isset($_GET['pmid']))
	{
		...
		elseif (!isset($_GET['kstart']))
		{
			...
			list ($_GET['start']) = $pmxcFunc['db_fetch_row']($request);
			$pmxcFunc['db_free_result']($request);
	...
	if ($context['display_mode'] == 2)
	{
		...
		$request = $pmxcFunc['db_query']('pm_conversation_list', '
	...
	if (!empty($pms))
	{
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
		{
			...
			if ($row['id_member_to'] == $user_info['id'] && $context['folder'] != 'sent')
			{
				...
				while($row2 = $pmxcFunc['db_fetch_assoc']($request2))
				{
					$l_id = $row2['id_label'];
		...
		if ($context['display_mode'] != 0)
		{
			...
			$subjects_request = $pmxcFunc['db_query']('', '
		...
		// Execute the query!
		$messages_request = $pmxcFunc['db_query']('', '
...
{
	global $txt, $scripturl, $modSettings, $context, $messages_request, $memberContext, $recipients, $pmxcFunc;
	...
	{
		$subject = $pmxcFunc['db_fetch_assoc']($subjects_request);
		if (!$subject)
		{
			$pmxcFunc['db_free_result']($subjects_request);
			return false;
		}
	...
	// Reset the data?
	if ($reset == true)
		return @$pmxcFunc['db_data_seek']($messages_request, 0);

	// Get the next one... bail if anything goes wrong.
	$message = $pmxcFunc['db_fetch_assoc']($messages_request);
	if (!$message)
	{
		if ($type != 'subject')
			$pmxcFunc['db_free_result']($messages_request);
...
{
	global $context, $txt, $scripturl, $pmxcFunc;
	...
	if (isset($context['search_params']['search']))
		$context['search_params']['search'] = $pmxcFunc['htmlspecialchars']($context['search_params']['search']);
	if (isset($context['search_params']['userspec']))
		$context['search_params']['userspec'] = $pmxcFunc['htmlspecialchars']($context['search_params']['userspec']);
...
function MessageSearch2()
{
	...
	{
		$userString = strtr($pmxcFunc['htmlspecialchars']($search_params['userspec'], ENT_QUOTES), array('&quot;' => '"'));
		...
		if (!empty($possible_users))
		{
			...
			foreach ($possible_users as $k => $v)
			{
				...
				if (!isset($where_params['real_name']))
					$where_params['real_name'] = $pmxcFunc['db_case_sensitive'] ? 'LOWER(real_name)' : 'real_name';
			}
			...
			if ($pmxcFunc['db_num_rows']($request) > $maxMembersToSearch)
				$userQuery = '';
			elseif ($pmxcFunc['db_num_rows']($request) == 0)
			{
				$userQuery = 'AND pm.id_member_from = 0 AND ({raw:pm_from_name} LIKE {raw:guest_user_name_implode})';
				$searchq_parameters['guest_user_name_implode'] = '\'' . implode('\' OR ' . ($pmxcFunc['db_case_sensitive'] ? 'LOWER(pm.from_name)' : 'pm.from_name') . ' LIKE \'', $possible_users) . '\'';
				$searchq_parameters['pm_from_name'] = $pmxcFunc['db_case_sensitive'] ? 'LOWER(pm.from_name)' : 'pm.from_name';
			}
			else
			{
				$memberlist = array();
	...
	foreach ($matches[1] as $index => $word)
		if ($word == '-')
		{
			$word = $pmxcFunc['strtolower'](trim($searchArray[$index]));
			if (strlen($word) > 0)
				$excludedWords[] = $word;
	...
	foreach ($tempSearch as $index => $word)
	{
		...
		{
			$word = substr($pmxcFunc['strtolower']($word), 1);
			if (strlen($word) > 0)
				$excludedWords[] = $word;
	...
	{
		$searchArray[$index] = $pmxcFunc['strtolower'](trim($value));
		if ($searchArray[$index] == '')
			unset($searchArray[$index]);
		else
		{
			// Sort out entities first.
			$searchArray[$index] = $pmxcFunc['htmlspecialchars']($searchArray[$index]);
		}
	}
...
function MessagePost()
{
	...
	loadTemplate('PersonalMessage');
	loadJavascriptFile('PersonalMessage.js', array('defer' => false), 'pmx_pms');
	...
	if (!empty($modSettings['pm_posts_per_hour']) && !allowedTo(array('admin_forum', 'moderate_forum', 'send_mail')) && $user_info['mod_cache']['bq'] == '0=1' && $user_info['mod_cache']['gq'] == '0=1')
	{
		...
		list ($postCount) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
	...
	if (!empty($_REQUEST['pmsg']))
	{
		...
		$isReceived = $pmxcFunc['db_num_rows']($request) != 0;
		$pmxcFunc['db_free_result']($request);
		...
		$row_quoted = $pmxcFunc['db_fetch_assoc']($request);
		$pmxcFunc['db_free_result']($request);
		...
		if ($_REQUEST['u'] == 'all' && isset($row_quoted))
		{
			...
			if ($row_quoted['id_member'] != $user_info['id'])
				$context['recipients']['to'][] = array(
					'id' => $row_quoted['id_member'],
					'name' => $pmxcFunc['htmlspecialchars']($row_quoted['real_name']),
				);
...
function messagePostError($error_types, $named_recipients, $recipient_ids = array())
{
	...
	global $pmxcFunc, $user_info, $sourcedir;
	...
	$context['subject'] = isset($_REQUEST['subject']) ? $pmxcFunc['htmlspecialchars']($_REQUEST['subject']) : '';
	$context['message'] = isset($_REQUEST['message']) ? str_replace(array('  '), array('&nbsp; '), $pmxcFunc['htmlspecialchars']($_REQUEST['message'])) : '';
...
function MessagePost2()
{
	...
	global $user_info, $modSettings, $pmxcFunc;
		...
		if (!empty($_REQUEST[$recipientType]))
		{
			...
			foreach ($namedRecipientList[$recipientType] as $index => $recipient)
			{
				if (strlen(trim($recipient)) > 0)
					$namedRecipientList[$recipientType][$index] = $pmxcFunc['htmlspecialchars']($pmxcFunc['strtolower'](trim($recipient)));
				else
			...
			if (!empty($namedRecipientList[$recipientType]))
			{
				...
				{
					$testNames = array(
						$pmxcFunc['strtolower']($member['username']),
						$pmxcFunc['strtolower']($member['name']),
						$pmxcFunc['strtolower']($member['email']),
					);
	...
	elseif (!empty($modSettings['max_messageLength']) && $pmxcFunc['strlen']($_REQUEST['message']) > $modSettings['max_messageLength'])
	...
	else
	{
		...
		if ($pmxcFunc['htmltrim'](strip_tags(parse_bbc($pmxcFunc['htmlspecialchars']($message, ENT_QUOTES), false), '<img>')) === '' && (!allowedTo('admin_forum') || strpos($message, '[html]') === false))
			$post_errors[] = 'no_message';
	}
	...
	if (isset($_REQUEST['preview']))
	{
		...
		$context['preview_subject'] = $pmxcFunc['htmlspecialchars']($_REQUEST['subject']);
		$context['preview_message'] = $pmxcFunc['htmlspecialchars']($_REQUEST['message'], ENT_QUOTES);
...
{
	global $context, $user_info, $options, $pmxcFunc;
	...
	if (!empty($to_label) && $context['folder'] == 'inbox')
	{
		...
		if ($context['display_mode'] == 2)
		{
			...
			while ($other_pms = $pmxcFunc['db_query']($get_pms))
			{
				$to_label[$other_pms['id_pm']] = $to_label[$other_pms['id_pm_head']];
			}

			$pmxcFunc['db_free_result']($get_pms);
		}
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
		{
			...
			elseif ($type == 'rem')
			{
				...
				list ($num_labels) = $pmxcFunc['db_fetch_assoc']($request2);
...
function markMessages($personal_messages = null, $label = null, $owner = null)
{
	...
	if ($personal_messages === null && $label !== null && $label != '-1')
	{
		...
		$get_messages = $pmxcFunc['db_query']('', '
			SELECT id_pm
			FROM {db_prefix}pm_labeled_messages
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($get_messages))
		{
			$personal_messages[] = $row['id_pm'];
		}

		$pmxcFunc['db_free_result']($get_messages);
	}
	elseif ($label = '-1')
	{
	...
	if ($pmxcFunc['db_affected_rows']() > 0)
	{
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($result))
		{
			...
			while ($row2 = $pmxcFunc['db_fetch_assoc']($result2))
			{
				$this_labels[] = $row2['id_label'];
			}

			$pmxcFunc['db_free_result']($result2);
...
function ManageLabels()
{
	...
	if (isset($_POST[$context['session_var']]))
	{
		...
		if (isset($_POST['add']))
		{
			$_POST['label'] = strtr($pmxcFunc['htmlspecialchars'](trim($_POST['label'])), array(',' => '&#044;'));

			if ($pmxcFunc['strlen']($_POST['label']) > 30)
				$_POST['label'] = $pmxcFunc['substr']($_POST['label'], 0, 30);
			if ($_POST['label'] != '')
			{
		...
		elseif (isset($_POST['save']) && !empty($_POST['label_name']))
		{
			foreach ($the_labels as $id => $name)
			{
				...
				{
					$_POST['label_name'][$id] = trim(strtr($pmxcFunc['htmlspecialchars']($_POST['label_name'][$id]), array(',' => '&#044;')));

					if ($pmxcFunc['strlen']($_POST['label_name'][$id]) > 30)
						$_POST['label_name'][$id] = $pmxcFunc['substr']($_POST['label_name'][$id], 0, 30);
		...
		if (!empty($labels_to_remove))
		{
			...
			$get_stranded_pms = $pmxcFunc['db_query']('', '
				SELECT pmr.id_pm
				FROM {db_prefix}pm_recipients AS pmr
			...
			while ($row = $pmxcFunc['db_fetch_assoc']($get_stranded_pms))
			{
				$stranded_messages[] = $row['id_pm'];
			}

			$pmxcFunc['db_free_result']($get_stranded_pms);
...
function ReportMessage()
{
	...
	global $user_info, $language, $modSettings, $pmxcFunc;
	...
	else
	{
		...
		if ($pmxcFunc['db_num_rows']($request) == 0)
			fatal_lang_error('no_access', false);
		list ($subject, $body, $time, $memberFromID, $memberFromName) = $pmxcFunc['db_fetch_row']($request);
		$pmxcFunc['db_free_result']($request);
		...
		while ($row = $pmxcFunc['db_fetch_assoc']($request))
		{
			...
			if (!isset($messagesToSend[$cur_language]))
			{
				...
				$messagesToSend[$cur_language] = array(
					'subject' => ($pmxcFunc['strpos']($subject, $txt['pm_report_pm_subject']) === false ? $txt['pm_report_pm_subject'] : '') . un_htmlspecialchars($subject),
					'body' => $report_body,
					'recipients' => array(
						'to' => array(),
						'bcc' => array()
...
function ManageRules()
{
	...
	elseif (isset($_GET['save']))
	{
		...
		// Name is easy!
		$ruleName = $pmxcFunc['htmlspecialchars'](trim($_POST['rule_name']));
		if (empty($ruleName))
		...
		foreach ($_POST['ruletype'] as $ind => $type)
		{
			...
			if ($type == 'mid')
			{
				...
				list ($memID) = $pmxcFunc['db_fetch_row']($request);
				$pmxcFunc['db_free_result']($request);
			...
			elseif (in_array($type, array('sub', 'msg')) && trim($_POST['ruledef'][$ind]) != '')
				$criteria[] = array('t' => $type, 'v' => $pmxcFunc['htmlspecialchars'](trim($_POST['ruledef'][$ind])));
		}
...
{
	global $user_info, $pmxcFunc, $context, $options;
...
function LoadRules($reload = false)
{
	...
	while ($row = $pmxcFunc['db_fetch_assoc']($request))
	{
		$context['rules'][$row['id_rule']] = array(
			'id' => $row['id_rule'],
			'name' => $row['rule_name'],
			'criteria' => pmx_json_decode($row['criteria'], true),
			'actions' => pmx_json_decode($row['actions'], true),
...
function isAccessiblePM($pmID, $validFor = 'in_or_outbox')
{
	...
	$validationResult = $pmxcFunc['db_fetch_assoc']($request);
	$pmxcFunc['db_free_result']($request);
