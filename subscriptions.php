...
$member_info = $pmxcFunc['db_fetch_assoc']($request);
...
$subscription_info = $pmxcFunc['db_fetch_assoc']($request);
$pmxcFunc['db_free_result']($request);
...
{
	global $modSettings, $notify_users, $pmxcFunc;
	...
	if (!empty($_POST))
	{
		foreach ($_POST as $key => $val)
			$text .= '<br>' . $pmxcFunc['htmlspecialchars']($key) . ': ' . $pmxcFunc['htmlspecialchars']($val);
	}
