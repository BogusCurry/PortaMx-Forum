...
function template_edit_style()
{
	...
	<div id="admincenter">
		<script>
			...
			function navigatePreview(url)
			{
				...
				myDoc.onreadystatechange = function ()
				{
					...
					if (myDoc.responseText != null && myDoc.status == 200)
					{
						...
						tempImage.src = pmx_prepareScriptUrl(pmx_scripturl) + "action=admin;area=theme;sa=edit;theme=', $settings['theme_id'], ';preview;" + (new Date().getTime());
			...
			function refreshPreview(check)
			{
				...
				refreshPreviewCache = refreshPreviewCache.replace(/url\(\.\.\/images/gi, "url(" + pmx_images_url);
	...
	echo '
				...
				if (!identical)
				{
					...
					// Replace the paths for images.
					preview_sheet = preview_sheet.replace(/url\(\.\.\/images/gi, "url(" + pmx_images_url);
