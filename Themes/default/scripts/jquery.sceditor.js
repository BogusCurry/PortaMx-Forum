	...
	'img', {
		...
		format: function (element, content) {
			...
			if (element.attr('data-attachment'))
			{
				...
				if (element.attr('expand'))
					attribs += ' expand=' + element.attr('expand');
	...
	'quote', {
		...
		format: function (element, content) {
			...
			do {
				...
				content = content.replace(/\n$/, '');
				content = content.replace(/\[\/qoute\]\n/, '[/quote]');
			} while (cLen != content.length);
		...
		html: function (element, attrs, content) {
			...
			do {
				...
				content = content.replace(/<br[^>]*>$/, '');
				content = content.replace(/<\/blockquote><br[^>]*>/, '</blockquote>');
			} while (cLen != content.length);
			...
			attrs.author = typeof(attrs.author) !== "undefined" ? attrs.author : '';
			author = attrs.author;
			if (author == '' && sDate == '')
				author = bbc_quote;
			...
			return '<blockquote author="' + attrs.author + '" date="' + attrs.date + '" link="' + attr_link + '"><cite>' + author + ' ' + sDate + '</cite>' + content + '</blockquote>';
		}
	}
);
