function pmx_StatsCenter(oOptions)
{
	this.opt = oOptions;
...
pmx_StatsCenter.prototype.init = function ()
{
	...
	for (var i = 0, n = aRows.length; i < n; i++)
	{
		...
		if ((aResults = this.opt.reYearPattern.exec(aRows[i].id)) != null)
		{
			...
			oCurYear.oToggle = new pmxc_Toggle({
				bToggleEnabled: true,
				...
				aSwapImages: [
					{
						...
						srcExpanded: pmx_images_url + '/' + this.opt.sYearImageExpanded,
						altExpanded: '-',
						srcCollapsed: pmx_images_url + '/' + this.opt.sYearImageCollapsed,
						altCollapsed: '+'
					}
				],
				aSwapLinks: [
		...
		else if ((aResults = this.opt.reMonthPattern.exec(aRows[i].id)) != null)
		{
			...
			oCurMonth.oToggle = new pmxc_Toggle({
				bToggleEnabled: true,
				...
				aSwapImages: [
					{
						...
						srcExpanded: pmx_images_url + '/' + this.opt.sMonthImageExpanded,
						altExpanded: '-',
						srcCollapsed: pmx_images_url + '/' + this.opt.sMonthImageCollapsed,
						altCollapsed: '+'
					}
				],
				aSwapLinks: [
...
pmx_StatsCenter.prototype.onBeforeCollapseMonth = function (oToggle)
{
...
pmx_StatsCenter.prototype.onBeforeExpandMonth = function (oToggle)
{
	...
	{
		// Make the xml call
		sendXMLDocument.call(this, pmx_prepareScriptUrl(pmx_scripturl) + 'action=stats;expand=' + oToggle.opt.sMonthId + ';xml', '', this.onDocReceived);
...
pmx_StatsCenter.prototype.onDocReceived = function (oXMLDoc)
{
