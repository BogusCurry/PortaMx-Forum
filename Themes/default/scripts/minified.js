...
function reqOverlayDiv(desktopURL,sHeader,sIcon){if(typeof(eclInMaintenace)!=='undefined'&&eclInMaintenace)
return !1;var eclcook=smfCookie('test','eclauth','','ecl');var sAjax_indicator='<div class="centertext"><img src="'+smf_images_url+'/loading_sm.gif"></div>';var sIcon=smf_images_url+'/'+(typeof(sIcon)=='string'?sIcon:'helptopics.png');var sHeader=typeof(sHeader)=='string'?sHeader:help_popup_heading_text;var oContainer=new smc_Popup({heading:sHeader,content:sAjax_indicator,icon:sIcon});var oPopup_body=$('#'+oContainer.popup_id).find('.popup_content');$.ajax({url:desktopURL,type:"GET",dataType:"html",beforeSend:function(){},success:function(data,textStatus,xhr){var help_content=$('<div id="temp_help">').html(data).find('a[href$="self.close();"]').hide().prev('br').hide().parent().html();oPopup_body.html(help_content)},error:function(xhr,textStatus,errorThrown){oPopup_body.html((typeof(eclErrorTxt)!='undefined'&&eclcook=='')?eclErrorTxt:textStatus)}});return !1}
...
return;if('oCookieOptions' in this.opt&&this.opt.oCookieOptions.bUseCookie){var cookieValue=smfCookie('get',this.opt.oCookieOptions.sCookieName);this.opt.bCurrentlyCollapsed=cookieValue=='1'}
...
smfCookie('set',this.opt.oCookieOptions.sCookieName,this.bCollapsed|0);if(!bInit&&'oThemeOptions' in this.opt&&this.opt.oThemeOptions.bUseThemeSettings)
...
$(item).css('display','block');$(item).css('top',$(this).offset().top+$(this).height());$(item).css('left',Math.max($(this).offset().left-$(item).width()+$(this).outerWidth(),0));$(item).height($(item).find('div:first').height())});$(document).click(function(){$(item).css('display','none')})});$(document).on('click','.you_sure',function(){var custom_message=$(this).attr('data-confirm');return confirm(custom_message?custom_message.replace(/-n-/g,"\n"):smf_you_sure)});$('.smf_select_text').on('click',function(e){e.preventDefault();var actOnElement=$(this).attr('data-actonelement');return typeof actOnElement!=="undefined"?smfSelectText(actOnElement,!0):smfSelectText(this)})});function smfCookie(sMode,sName,sValue,sType){sType=sType==undefined?'':sType;sValue=sValue==undefined?'':sValue;var sResult='';$.ajax({type:'GET',async:!1,url:smf_scripturl+'?jscook',data:{mode:sMode,name:sName,value:sValue,type:sType},success:function(data){sResult=data}});return sResult};$(document).ready(function(){$('ul.dropmenu, ul.quickbuttons').superfish({delay:250,speed:100,sensitivity:8,interval:50,timeout:1});$('.preview').SMFtooltip();$('a.bbc_link img.bbc_img').parent().css('border','smf_tooltip')});function smf_codeBoxFix(){var codeFix=$('code');$.each(codeFix,function(index,tag){if(is_webkit&&$(tag).height()<20)
