	...
	foreach ($context['posts'] as $post)
	{
		...
		if (isset($_GET['images']) && !empty($context['printattach'][$post['id_msg']]))
		{
			...
			foreach ($context['printattach'][$post['id_msg']] as $attach)
				echo '
					<img width="' . $attach['width'] . '" height="' . $attach['height'] . '" src="', $scripturl . '?action=dlattach;topic=' . $topic . '.0;attach=' . $attach['id_attach'] . ';image" alt="" title="" oncontextmenu="return false">';
		}
